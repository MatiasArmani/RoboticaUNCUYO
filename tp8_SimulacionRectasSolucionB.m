clc; clear; close all;

% Definir los puntos iniciales de la trayectoria en el espacio cartesiano
P1 = [0, 0.7, 0.2];
P2 = [0.5, 0.5, 0.5];
P3 = [0.7, 0.2, -0.2];

% Definir el robot (modelo de 6 grados de libertad)
d1 = 0.150; d6 = 0.150; d3 = 0.500; a4 = 0.500; dEspatula = 0.0; hBase = 0;

dh = [
    0   d1    0    pi/2   0;
    0    0    0   -pi/2   0;
    0   d3    0    pi/2   0;
    0    0   a4   -pi/2   0;
    0    0    0    pi/2   0;
    0   d6    0       0   0];

R = SerialLink(dh,'name','Robot Heladero','plotopt',{'scale',0.8,'jointdiam',1.4,'jointcolor',[0 0.3 0.5],'name','tilesize',0.2,'tile1color',[1 1 1],'noname','lightpos',[0 0 10]});
R.base = transl(0, 0, hBase);
R.tool = transl(0, 0, dEspatula);
R.qlim(1:6, :) = [-180 180; -180 180; -180 180; -180 180; -180 180; -180 180]*pi/180;

% Parámetros de interpolación en el espacio cartesiano
n_puntos_intermedios = 10;  % Número de puntos intermedios entre cada par de puntos principales
trayectoria_cartesiana = [P1; P2; P3];  % Trayectoria original con puntos principales

% Inicializar matriz para almacenar los puntos interpolados en el espacio cartesiano
trayectoria_interpolada = [];
% Generar puntos intermedios en el espacio cartesiano entre los puntos
% principales
for i = 1:length(trayectoria_cartesiana)-1
    P_traj = jtraj(trayectoria_cartesiana(i,:),trayectoria_cartesiana(i+1,:),n_puntos_intermedios);
    trayectoria_interpolada = [trayectoria_interpolada; P_traj];
end

% % Transponer la trayectoria interpolada para facilitar el cálculo de la cinemática inversa
% trayectoria_interpolada = trayectoria_interpolada';

% Inicializar matriz para almacenar los ángulos articulares de cada punto interpolado
q_trayectoria = zeros(size(trayectoria_interpolada, 1), 6);
q0 = [0,0,0,0,0,0];  % Ángulo inicial del robot

% Calcular la cinemática inversa para cada punto interpolado en el espacio cartesiano
for i = 1:size(trayectoria_interpolada, 1)
    pos_actual = trayectoria_interpolada(i, :);
    T_actual = transl(pos_actual);  % Matriz de transformación para el punto actual
    q_trayectoria(i, :) = cinInversa(R, T_actual, 1, dh, q0);  % Cinemática inversa para el punto interpolado
    q0 = q_trayectoria(i, :);  % Actualizar el ángulo inicial para la siguiente iteración
end

% Configuración del gráfico 3D para el robot
figure;
R.plot(q_trayectoria(1, :));  % Mostrar la posición inicial del robot
hold on;

% Ploteo de los puntos principales de la trayectoria en el espacio
plot3(P1(1), P1(2), P1(3), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
plot3(P2(1), P2(2), P2(3), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
plot3(P3(1), P3(2), P3(3), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');

% Inicializar las matrices de la curva para graficar la trayectoria interpolada
curve_x = [];
curve_y = [];
curve_z = [];

% Animación de la trayectoria y ploteo de la curva interpolada en el espacio cartesiano
dt = 0.01;
for i = 1:size(q_trayectoria, 1)
    R.plot(q_trayectoria(i, :));  % Dibujar el robot en cada posición de la trayectoria

    % Obtener la posición del efector final
    T_efector = R.fkine(q_trayectoria(i, :));  % Cinemática directa para obtener posición

    % Extraer las coordenadas x, y, z del efector final
    curve_x = [curve_x, T_efector.t(1)];
    curve_y = [curve_y, T_efector.t(2)];
    curve_z = [curve_z, T_efector.t(3)];

    % Graficar cada nuevo punto en la curva roja
    plot3(curve_x(end), curve_y(end), curve_z(end), 'b.', 'MarkerSize', 5);
    drawnow;  % Actualizar la gráfica en cada iteración

    pause(dt);  % Controlar la velocidad de la animación
end

% Ajustes finales del gráfico
xlabel('X');
ylabel('Y');
zlabel('Z');
grid on;
axis equal;
title('Simulación de la trayectoria interpolada en el espacio cartesiano entre puntos principales');

%% Grafico de posiciones articulares
figure
q_traj_total = q_trayectoria;
n = size(q_traj_total,1);
t = 0:dt:(n-1)*dt;
grid on
hold on
colores = ['b','g','c','m','k','y'];
for i=1:6
plot(t,q_traj_total(:,i),colores(i))
end
legend('q1','q2','q3','q4','q5','q6');
xlabel('Tiempo (s)');
ylabel('Angulo articular (rad)');
title('Posiciones articulares');
%% Grafico de velocidades articulares
figure
grid on
hold on
dq = DerivacionNumerica(q_traj_total,t);
for i=1:6
plot(t,dq(:,i),colores(i))
end
legend('dq1','dq2','dq3','dq4','dq5','dq6');
xlabel('Tiempo (s)');
ylabel('Velocidad articular (rad/s)');
title('Velocidades de coordenadas articulares');
%% Grafico de aceleraciones articulares
ddq = DerivacionNumerica(dq,t);
figure
grid on
hold on
for i=1:6
plot(t,ddq(:,i),colores(i))
end
legend('ddq1','ddq2','ddq3','ddq4','ddq5','ddq6');
xlabel('Tiempo (s)');
ylabel('Aceleracion articular (rad/s^2)');
title('Aceleraciones de coordenadas articulares');
