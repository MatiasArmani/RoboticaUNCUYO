...
% Es el mismo codigo del ejercicio 1, solo que se verifica usando
% la herramienta del toolbox, la cual esta implementada directamente
% en el imprimirMatriz
...
imprimirMatrices(listMatrix1,R1,q1);
disp('Matrices de transformacion para q2');
imprimirMatrices(listMatrix2,R2,q2);
disp('Matrices de transformacion para q3');
imprimirMatrices(listMatrix3,R3,q3);
disp('Matrices de transformacion para q4');
imprimirMatrices(listMatrix4,R4,q4);
...
function imprimirMatrices(listMatrix,R,q)
    [~,~,n] = size(listMatrix);
    for i = 1:n-1
        fprintf('%.0fM%.0f es la posición y orientación del sistema %d respecto del %d:\n',i-1 , i, i, i-1);
        disp('Calculadas con matrices de transformacion');
        disp(listMatrix(:,:,i));
        disp('Calculadas usando el toolbox');
        disp(R.links(i).A(q(i)));
    end
    fprintf('%.0fM%.0f es la posición y orientación del sistema %d respecto del %d:\n', 0,  n-1, n-1, 0);
    disp('Calculadas con matrices de transformacion');
    disp(listMatrix(:,:,n));
